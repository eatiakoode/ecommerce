...
const createSize = asyncHandler(async (req, res) => {
  ...
  if (!name || !value) {
    return res.status(400).json({
      message: "Nmae & value are required",
    });
  }
  ...
  const size = await Size.create({ name, value, isActive });
...
// ➤ Get Size by Slug
const getSizeBySlug = asyncHandler(async (req, res) => {
  const { slug } = req.params;
  const size = await Size.findOne({ slug });
