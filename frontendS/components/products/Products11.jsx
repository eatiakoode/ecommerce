...
export default function Products11() {
  ...
  const buildQuery = () => {
    ...
    if (availability !== 'All') params.append('availability', availability.value);
    if (color !== 'All') params.append('color', color.name);
    if (size !== 'All' && size !== 'Free Size') params.append('size', size);
    if (activeFilterOnSale) params.append('onSale', 'true');
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/frontend/product/filter${query ? `?${query}` : ''}`);
        const result = await res.json();
        console.log(result.data)
        setProducts(result?.data || []);
        dispatch({ type: "SET_FILTERED", payload: result?.data || [] });
      } catch (err) {
