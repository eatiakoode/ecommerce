...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const apiData: { month: number; totalSales: number; totalOrders: number }[] = res.data;
      const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

      const labels = apiData.map((item) => monthNames[item.month - 1]);
      const sales = apiData.map((item) => item.totalSales);
      const orders = apiData.map((item) => item.totalOrders);
...
// }, []);
  ...
  return (
    <Card className="shadow-lg transition-transform duration-200 hover:scale-105 hover:shadow-2xl bg-gradient-to-br from-blue-50 via-white to-green-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-900 animate-fadeIn">
      ...
      <CardContent className="pb-2">
        <Tabs defaultValue="sales">
          ...
          <TabsContent value="sales" className="relative h-60 flex items-center justify-center">
            {mounted && !loading ? (
              <Line
                data={{
                  labels,
          ...
          <TabsContent value="orders" className="relative h-60 flex items-center justify-center">
            {mounted && !loading ? (
              <Line
                data={{
                  labels,
