...
import { X, Plus, Upload, Image as ImageIcon, ChevronDown, Check } from "lucide-react";
...
export default function AddProductForm() {
  ...
  const { brands, loading: brandsLoading, refreshBrands } = useBrands() as { brands: Option[]; loading: boolean; refreshBrands: () => void };
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        // Set colors from API or use defaultColors if empty
  ...
  const handleColorSelect = (color: Option) => {
  ...
  const handleSizeSelect = (size: Option) => {
  ...
  const onSubmit = async (data: any) => {
    console.log('Submitting product:', data, { selectedColors, selectedSizes, selectedCategories, images });
    // Validation
    ...
    try {
      ...
      for (const color of selectedColors) {
        if (!color._id || color._id.startsWith('default-')) {
          // Save to backend and get the new _id
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <Label className="text-sm font-medium text-gray-700">
            ...
            {brandsLoading && <span className="text-xs text-gray-500 ml-2">(Loading...)</span>}
          </Label>
          <div className="relative mb-4">
            <select
              {...register("brand", { required: "Brand is required" })}
              className="w-full mt-1 h-11 border border-gray-300 rounded-md px-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            >
              ...
              {brands.length > 0 ? (
                ...
                !brandsLoading && <option disabled>No brands available</option>
              )}
            </select>
            {errors.brand && (
        ...
        <div>
                  ...
                  />
                  {errors.MRP && (
                    <p className="text-red-500 text-sm mt-1">{String(errors.MRP.message)}</p>
                  )}
        </div>
        <div>
                  <Label className="text-sm font-medium text-gray-700">
                  ...
                  />
                  {errors.sellingPrice && (
                    <p className="text-red-500 text-sm mt-1">{String(errors.sellingPrice.message)}</p>
                  )}
        </div>
        <div>
                  <Label className="text-sm font-medium text-gray-700">
                  ...
                  />
                  {errors.quantity && (
                    <p className="text-red-500 text-sm mt-1">{String(errors.quantity.message)}</p>
                  )}
                </div>
        </div>
      </div>
        ...
        <div>
                  ...
                  {errors.longDescription && (
                    <p className="text-red-500 text-sm mt-1">{String(errors.longDescription.message)}</p>
                  )}
                </div>
        </div>
      </div>
      ...
      <div>
                ...
                <Input 
                  {...register("tags")} 
                  placeholder="Enter tags separated by commas (e.g., summer, casual, trending)"
                  className="mt-1 h-11"
                />
